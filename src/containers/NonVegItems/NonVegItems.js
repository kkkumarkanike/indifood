...
class NonVegItems extends Component {
    ...
    render() { 
        ...
        allIds.map(id =>{
            if (all[id].category === "non-veg" ||  all[id].category === "special"){
                allItems[id] = all[id];
            }
        })
            ...
            filteredItems.map(sub_cat =>{
                Object.keys(allItems).map(id => {
                    if  (allItems[id].sub_cat === sub_cat){
                        if (allItems[id].category === "non-veg" || allItems[id].category === "special"){
                ...
                return (
                        ...
                        <div className="filtered-food-card">
                            <Link to={'/details/'+item} >
                                <img src={finalFilteredItems[item].img} alt="image" className="food-image"/>
                            </Link>
                            <div className="filtered-details">
                                <p style={{margin: 0, color : "#000"}}><b>{finalFilteredItems[item].title}</b><br/>
                                    <span className="filtered-desc">{finalFilteredItems[item].desc}</span>
                                </p>
                                <button className="left"><i className="fa fa-rupee"></i>&nbsp;{finalFilteredItems[item].price}
                                </button>
                             <button className="right" onClick={() => this.onAddItemToCart(finalFilteredItems[item])}><b>ADD</b></button>
            ...
            mobileItems = allItemsIds.map(item =>{
                return (
                    <div className="mobile-single-item">
                        <img src={finalFilteredItems[item].img} alt="" className="mobi-image"/>
                        <div className="mobi-item-details">
                            <h5 className="mobi-item-head">{finalFilteredItems[item].title}</h5>
                            <p className="mobi-item-para">{finalFilteredItems[item].desc}</p>
                            <div className="rate-add">
                                <div className="rate-flex">
                                    <div>
                                        <button className="rate-btn"><i className="fa fa-rupee"></i>&nbsp;{finalFilteredItems[item].price}
                                        </button>
                                    </div>
                                </div>
                                <div className="add-flex">
                                    <div>
                                        <button className="add-btn" onClick={() => this.onAddItemToCart(finalFilteredItems[item])}>ADD</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <Aux>
                                            ...
                                            <div className="filter-block">
                                                <div className="filters">
                                                    <div className="type-flex">
                                                        <p><input type="checkbox"  name="rice"  onChange={this.handleFilters} />
                                                            <b><span className="txt hover" name="rice">Rice Items</span></b></p>
                                                    </div>
                                                    <div className="type-flex">
                                                        <p><input type="checkbox" name="curry" onChange={this.handleFilters} />
                                                            <b><span className="txt hover">Curries</span></b></p>
                                                    </div>
                                                    <div className="type-flex">
                                                        <p><input type="checkbox" name="roti" onChange={this.handleFilters} />
                                                            <b><span className="txt hover">Roties</span></b></p>
                                                    </div>
                                                    <div className="type-flex">
                                                        <p><input type="checkbox" name="starter" onChange={this.handleFilters} />
                                                            <b><span className="txt hover">Starters</span></b></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="vl"></div>
                                            <div className="filtered-items-block">
                                                <p className="txt">{Object.keys(finalFilteredItems).length} ITEMS</p>
                            ...
                            <div className="mobile">
                                <div className="mobile-banner">
                                    <p className="mobile-banner-head">Non Veg</p>
                                    <p className="mobile-banner-sub">All types of veg items are available here</p>
                                    <div className="hr-lite"></div>
                                    <div className="all-filters">
                  <p className="filter-text">Filters <FontAwesomeIcon icon={faFilter}/></p>
                  <div className="mobile-banner-details">
                    <div className="each each-rate" name="rice" onClick={() => this.handleMobileFilters('rice')}>
                      <div className="filter-opt" id="rice">
                      <p>Rice Items</p>
                      </div>
                    </div>
            
                    <div className="each each-rate" onClick={() => this.handleMobileFilters('curry')}>
                      <div className="filter-opt" id="curry">
                      <p>Curries</p>
                      </div>
                    </div>
                    <div className="each each-rate" onClick={() => this.handleMobileFilters('roti')}>
                      <div className="filter-opt" id="roti">
                      <p>Roti</p>
                      </div>
                    </div>
                    <div className="each each-rate" onClick={() => this.handleMobileFilters('starter')}>
                      ...
                      <p>Starters</p>
                                    ...
                                    <div className="hr-lite"></div>
                                    <p className="font-dim mar">{Object.keys(this.props.foodItems).length ? this.props.foodItems.length : "no"} items found</p>
                                    <br/>
